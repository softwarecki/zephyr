 ...
 */
static const uint32_t *dai_dmic_skip_coeff(const uint32_t *coeff, const int length, const bool packed)
{
	if (!packed) {
		return coeff + length;
	} else {
		...
		coeff++;
		return UINT_TO_POINTER(ROUND_UP(POINTER_TO_UINT(coeff) + sizeof(uint32_t) +
						3 * length, sizeof(uint32_t)));
	}
}
...
static int dai_nhlt_get_clock_div(struct dai_intel_dmic *dmic, const int dai_index, const int pdm)
{
	uint32_t val;
	...
	val = dai_dmic_read(dmic, base[pdm] + FIR_CHANNEL_REGS_SIZE * dai_index + FIR_CONFIG);
	...
	LOG_ERR("dai_index = %d, rate_div = %d, p_clkdiv = %d, p_mcic = %d, p_mfir = %d",
		dai_index, rate_div, p_clkdiv, p_mcic, p_mfir);
...
static int dai_nhlt_dmic_dai_params_get(struct dai_intel_dmic *dmic, const int clock_source)
{
	...
	rate_div = dai_nhlt_get_clock_div(dmic, dmic->dai_config_params.dai_index, first_pdm);
	if (rate_div < 0) {
		return rate_div;
	}
	...
	LOG_INF("io_clk %u, rate_div %d",
		adsp_clock_source_frequency(clock_source), rate_div);
...
static int dai_nhlt_dmic_dai_params_get(struct dai_intel_dmic *dmic)
{
	...
	clk_div = FIELD_GET(MIC_CONTROL_PDM_CLKDIV, mic_control[0]) + 2;

	cic_config = dai_dmic_read(dmic, base[0] + CIC_CONFIG);
	comb_count = FIELD_GET(CIC_CONFIG_COMB_COUNT, cic_config) + 1;

	fir_config = dai_dmic_read(dmic, base[0] +
				   dmic->dai_config_params.dai_index * FIR_CHANNEL_REGS_SIZE +
				   FIR_CONFIG);
	fir_decimation = FIELD_GET(FIR_CONFIG_FIR_DECIMATION, fir_config) + 1;

	LOG_INF("clkdiv = %d, mcic = %d, mfir = %d", clk_div, comb_count, fir_decimation);

	rate_div = clk_div * comb_count * fir_decimation;
	if (!rate_div) {
		LOG_ERR("zero clock divide or decimation factor");
		return -EINVAL;
	}
	...
	LOG_INF("io_clk %u, rate_div %d", adsp_clock_souce_frequency(dmic_cfg->clock_source),
		rate_div);
...
static void print_outcontrol(uint32_t val)
{
