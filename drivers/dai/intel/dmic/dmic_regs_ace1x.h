...
* the streams and channels associated with the DSP link DMA or GPDMA, if the FIFO port of the link IP
* is connected to the Audio Link Hub (DMICLCAP.SCMS = 1).
* When mapping to GPDMA, the STRMzRXDA should be used to read from the FIFO port for input stream;
* with the STRM field indicating the offset z; and the CHAN field will indicate which channel of the
* sample block the FIFO port should start producing.
*/
